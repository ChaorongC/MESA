...
def calculate_integration(X_list, y, feature_selected, classifiers):
    ...
    y_pred_all = []
    y_true = []
    cv_index = list(cv_method.split(X[0].T, y))
    scaler = StandardScaler()
    for run in range(len(cv_index)):
        train_index, test_index = cv_index[run]
        y_train, y_test = np.array(y)[train_index], np.array(y)[test_index]
        X_temp = [x.iloc[:, list(fea[run])] for x, fea in zip(X, feature_selected)]
        y_train, y_test = np.array(y)[train_index], np.array(y)[test_index]
        X_train_temp = [imputation_cv(X1, train_index, test_index, 0.9)[0] for X1 in X]
        X_test_temp = [imputation_cv(X1, train_index, test_index, 0.9)[1] for X1 in X]
        X_train = [
